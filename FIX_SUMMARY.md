...
- Added domain-only browsing analytics and explicitly excluded normal paths, queries, hashes, and page titles.
- Added client-generated PDF security reports with security score, trends, recommendations, and domain evidence.
- Added an authenticated, rate-limited Gemini assistant with multi-turn session chat, proactive remediation prompts, and transient image analysis.
- Restricted every admin API to the exact configured `ADMIN_EMAIL` owner and prevented promotion of other accounts.
