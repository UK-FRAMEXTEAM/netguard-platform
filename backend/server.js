...
const siteProtectionRoutes = require('./routes/siteProtection');
...
// The assistant accepts one transient base64 screenshot up to 4 MB. Protected-site
// telemetry is intentionally small and is validated again by its own router.
app.use(express.json({ limit: '6mb' }));
app.use(express.urlencoded({ extended: false, limit: '100kb' }));

// Protected websites have their own origin validation and CORS response because
// their origins are registered dynamically in MongoDB.
app.use('/api/site', siteProtectionRoutes);
...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    perSiteProtectionControls: true,
    automaticSitePostureScans: true,
    recaptchaProtection: Boolean(process.env.RECAPTCHA_SITE_KEY && process.env.RECAPTCHA_SECRET_KEY),
